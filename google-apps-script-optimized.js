    ...
    if (data.property_images && Array.isArray(data.property_images) && data.property_images.length > 0) {
      ...
      for (let batch = 0; batch < data.property_images.length; batch += batchSize) {
        ...
        if (!checkExecutionTime()) {
          Logger.log('⚠️ Approaching execution time limit, stopping at ' + processedFiles + ' files');
          break;
        }
        ...
        for (let i = batch; i < batchEnd; i++) {
          ...
          try {
            ...
            processedFiles++;
            Logger.log('✅ Image uploaded (' + processedFiles + '/' + totalFiles + '): ' + fileData.name);
      ...
      for (let batch = 0; batch < data.documents.length; batch += batchSize) {
        ...
        for (let i = batch; i < batchEnd; i++) {
          ...
          try {
            ...
            processedFiles++;
            Logger.log('✅ Document uploaded (' + processedFiles + '/' + totalFiles + '): ' + fileData.name);
